...
namespace Fortis.Search
{
	...
	/// </summary>
	public class CustomGenericQueryable<TElement, TQuery> : GenericQueryable<TElement, TQuery>
	{
		public CustomGenericQueryable(Index<TElement, TQuery> index, QueryMapper<TQuery> queryMapper, IQueryOptimizer queryOptimizer, FieldNameTranslator fieldNameTranslator)
			: base(index, queryMapper, queryOptimizer, fieldNameTranslator) { }

		protected CustomGenericQueryable(Index<TQuery> index, QueryMapper<TQuery> queryMapper, IQueryOptimizer queryOptimizer, Expression expression, Type itemType, FieldNameTranslator fieldNameTranslator)
		...
		protected override TQuery GetQuery(Expression expression)
		{
			...
			IndexQuery rawQuery = new CustomExpressionParser(typeof(TElement), ItemType, FieldNameTranslator).Parse(expression);
		...
		public override IQueryable<TQueryElement> CreateQuery<TQueryElement>(Expression expression)
		{
			...
			return new CustomGenericQueryable<TQueryElement, TQuery>(Index, QueryMapper, QueryOptimizer, expression, ItemType, FieldNameTranslator);
		}
	}
}
